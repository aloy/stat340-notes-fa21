...
1. Use the below code to load the data and prepare it for use with JAGS.
    ...
    ```{r}
# Split data into standard (std) and color (col) groups
y <- stroop$Time
...
```
    ...
    Yes, the chains have converged. All of the trace plots have a clear center of mass and appear to be “white noise” around this center of mass. (In other words, they look like fuzzy catepillars.)
...
```
    ...
    All of the ACF plots appear to decay quite quickly, indicating there is not strong temporal correlation within a chain. Consequently, we should be exploring the parameter space in a relatively efficient manner.
...
1. Calculate the posterior mean and SD for each parameter.
    ...
    The mean and SD are given in the output of summary().
    
    ```{r}
summary(stroop_samples)[,c("Mean", "SD")]
```
...
1. Calculate a 95% credible interval for $\delta$. What does this say about the difference in average completion time between groups?
    ...
    You could use the 95% credible interval produced by `summary()`; however, this is a highest posterior density credible interval, which we have not discussed yet. To get an equal-tail credible interval, we can use `quantile()` as before:
    
    ```{r}
quantile(unlist(stroop_samples$mcmc[,"delta"]), probs = c(0.025, 0.975))
```
...
$$
    ...
    For $i=1,\ldots,5000$ do the following:

    i. Draw $\mu^{(i)}$ from $\mathcal{N} \left(\dfrac{\phi^{(i-1)} \sum_{i=1}^{n} Y_i + \phi^{(i-1)} \sum_{j=n+1}^{n+m} (Y_j - \delta\phi^{(i-1)})}{\phi^{(i-1)}(n+m) + 1/100^2},\ \dfrac{1}{\phi^{(i-1)}(n+m) + 1/100^2} \right)$.

    ii. Draw $\delta^{(i)}$ from $\mathcal{N} \left( \dfrac{\phi^{(i-1)}\sum_{i=n+1}^{n+m} (Y_i - \mu^{(i)})}{\phi^{(i-1)} m + 1/100^2} ,\ \dfrac{1}{\phi^{(i-1)} m + 1/100^2} \right)$.

    iii. Draw $\phi^{(i)}$ from ${\rm Gamma} \left( 0.01 + \frac{n+m}{2}, \ 0.01 + \dfrac{\sum_{i=1}^{n} (Y_i - \mu^{(i)})^2}{2} + \dfrac{\sum_{j=n+1}^{n+m} (Y_j - \mu^{(i)}-\delta^{(i)})^2}{2} \right)$