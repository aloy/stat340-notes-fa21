...
Condition on $Y=3$, simulate an X 
...
\mu & \sim \mathcal{N}(\mu_0, \sqrt{1/\sigma_0}) &\\
...
How can we do this *efficiently* sample from this posteriorin two dimensions?
]
...
.pull-left[
```{r echo=FALSE, fig.height = 4.5, fig.width = 4.5, out.width = "65%", message=FALSE, fig.align='center'}
as.data.frame(no_burn) %>%
  ggplot() +
  geom_histogram(aes(x = mu, y = ..density..), alpha = 0.7) +
  theme_light() +
...
as.data.frame(no_burn) %>%
  ggplot() +
  geom_histogram(aes(x = 1/phi, y = ..density..), alpha = 0.7) +
  theme_light() +
