...
slr_draws <- run.jags(slr_model, monitor = c("beta0", "beta1", "sigma"), data = list(y = new_coffee$total_cup_points, x = new_coffee$aroma, n = nrow(new_coffee)), n.chains = 3, burnin = 5000, adapt = 1000, sample = 5000, thin = 80)
```

```{r echo=FALSE}
print(slr_draws, digits = 3)
```
...
```{r yt3, echo=FALSE, fig.height = 3, fig.width = 4, fig.align='center', out.width = "50%", message=FALSE}
library(tidybayes)
post_draws <- tidy_draws(slr_draws$mcmc)

beta_draws <- post_draws %>% select(-sigma)
mu_link <- function(aroma) beta_draws[["beta0"]] + beta_draws[["beta1"]] * aroma

x_grid <- seq(6.5, 8.5, by = .1)
yhat_draws<- sapply(x_grid, mu_link)

yhat_mean <- apply(yhat_draws, 2, mean)

yhat_ci <- apply(yhat_draws, 2, quantile, probs = c(0.05, 0.95))

ggplot(new_coffee) +
  geom_point(aes(x = aroma, y = total_cup_points), shape = 1) +
  geom_line(
    data = data.frame(x = x_grid, y = yhat_mean),
    aes(x = x, y = y),
    color = "blue") +
  geom_ribbon(
    data = data.frame(x = x_grid, ymin = yhat_ci[1,], ymax = yhat_ci[2,]),
...
ggsave(filename = "22-slr-checking_files/figure-html/yt3-1.pdf")
```
...
```{r echo=FALSE, fig.height = 3, fig.width = 4, fig.align='center', out.width = "50%"}
...
```{r echo=FALSE, fig.height = 3, fig.width = 4, fig.align='center', out.width = "50%"}
ggplot(data = resids, aes(sample = resid)) +
  geom_qq(alpha = 0.5) +
  stat_qq_line(linetype = 2, color = "dodgerblue") +
  theme_light() +
...
mu_link <- function(aroma) {
  post_draws[["beta0"]] + post_draws[["beta1"]] * aroma
}
...
ggplot(data = NULL, aes(x = new_coffee$total_cup_points)) +
  geom_point(aes(y = y_means), alpha = 0.5) +
  geom_linerange(aes(ymin = y_pis[1,], ymax = y_pis[2,]), alpha = 0.5) +
  geom_abline(slope = 1, intercept = 0, color = "dodgerblue") +
  theme_light() +
  labs(x ="Observed cup points", y = "Predicted cup points")
```
...
pred_resids <- post_pred_data %>%
  mutate(
    ...
    resid_lo = y - y_lo,
    resid_hi = y - y_hi,
    x = jitter(new_coffee$aroma, factor = 2.5)
    )
...
ggplot(pred_resids, aes(x = x)) +
  ...
  labs(x = "Aroma grade", y = "Predictive residual") +
  theme_light()
